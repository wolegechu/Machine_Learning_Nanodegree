...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        randomAct = Random.choice(self.valid_actions)
        if self.learning is False :
            action = randomAct
        else:
            maxQ = self.get_maxQ(state)
            randomNum = Random.random()
            if randomNum < self.epsilon or maxQ is None:
                action = randomAct
            else:
                if len(maxQ[0]) == 1:
                    action = maxQ[0][0]
                else:
                    action = Random.choice(maxQ[0])
        return action
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            alpha = self.alpha
            self.Q[state][action] = (1 - alpha) * Qsa + alpha * (reward)
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning=True, alpha=0.4, epsilon=1)
